...
func _physics_process(delta: float) -> void:
	...
	if distance > aggro_range:
		...
		velocity = Vector2.ZERO
		_try_attack(offset.normalized())
	visual.set_motion(velocity)


func configure(config: Dictionary, new_instance_id: String, restored_health: float = -1.0) -> void:
	...
	attack_cooldown = float(attack_data.get("cooldown", attack_cooldown))
	_apply_collision_radius()
