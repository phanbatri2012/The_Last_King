...
static func apply_damage(target: HealthComponent, requested_damage: float, context: Dictionary = {}) -> Dictionary:
	var result := {
	...
	var applied := target.apply_resolved_damage(requested_damage, context)
