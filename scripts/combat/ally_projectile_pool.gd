class_name AllyProjectilePool
extends Node2D

const PROJECTILE_SCENE := preload("res://scenes/gameplay/ally_projectile.tscn")

@export_range(0, 128, 1) var prewarm_count := 24
@export_range(1, 256, 1) var maximum_count := 96

var _available: Array[AllyProjectile] = []
...
func request_projectile(request: Dictionary) -> AllyProjectile:
	if not _combat_enabled:
		return null
	var projectile: AllyProjectile
...
func set_combat_enabled(enabled: bool) -> void:
	...
	for projectile_value in _active.values().duplicate():
		var projectile := projectile_value as AllyProjectile
...
func _create_projectile() -> AllyProjectile:
	var projectile := PROJECTILE_SCENE.instantiate() as AllyProjectile
	if projectile == null:
		return null
...
func _on_projectile_resolved(projectile: AllyProjectile) -> void:
	_active.erase(projectile.get_instance_id())
	call_deferred("_recycle_projectile", projectile)


func _recycle_projectile(projectile: AllyProjectile) -> void:
